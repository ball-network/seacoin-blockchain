...
get_madmax_filename() {
  ...
  export SEA_PLOT="sea_plot"
  if [ "$KSIZE" = "k34" ]; then
    export SEA_PLOT="sea_plot_k34"
  fi
  SUFFIX=""
  if [ "$OS" = "macos" ]; then
    if [ "$ARCH" = "arm64" ]; then
  ...
  echo "${SEA_PLOT}-${MADMAX_VER}-${SUFFIX}"
}
