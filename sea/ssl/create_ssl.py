...
from sea.util.ssl_check import DEFAULT_PERMISSIONS_CERT_FILE, DEFAULT_PERMISSIONS_KEY_FILE
...
def get_sea_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "sea_ca.crt")
    key = pkg_resources.resource_string(__name__, "sea_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Sea"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("seacoin.vip")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Sea"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Sea CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def create_all_ssl(
    ...
    *,
    ...
    sea_ca_crt, sea_ca_key = get_sea_ca_crt_key()
    sea_ca_crt_path = ca_dir / "sea_ca.crt"
    sea_ca_key_path = ca_dir / "sea_ca.key"
    write_ssl_cert_and_key(sea_ca_crt_path, sea_ca_crt, sea_ca_key_path, sea_ca_key, overwrite=overwrite)
...
def main():
    return make_ca_cert(Path("./sea_ca.crt"), Path("./sea_ca.key"))
