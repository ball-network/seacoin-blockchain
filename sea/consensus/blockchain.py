...
from sea.consensus.block_body_validation import ForkInfo, validate_block_body
...
from sea.types.blockchain_format.proof_of_space import ProofOfSpace
...
from sea.types.stake_record import StakeRecord
...
from sea.wallet.puzzles.stake.drivers import match_stake_puzzle_by_coin_spend
...
class Blockchain(BlockchainInterface):
        ...
        try:
            ...
            for fetched_block_record in records:
                self.__height_map.update_height(
                    ...
                    fetched_block_record.farm_puzzle_hash,
                )
        ...
        for fetched_full_block, fetched_block_record in reversed(blocks_to_add):
            ...
            if coin_spends_res is not None:
                ...
                for coin in tx_additions:
                    if (coin.amount / MOJO_PER_XSEA).is_integer():
                ...
                for coin_spend in coin_spends_res:
                    ...
                    if stake_metadata is not None:
                        ...
                        if coin is not None:
                            if coin.amount < STAKE_FARM_MIN if stake_metadata.is_stake_farm else (
    ...
    async def get_stake_coefficient_new(self, height: uint32, stake_puzzle_hash: bytes32) -> uint64:
        ...
        log.debug(
            f"height: {height}, stake: {stake_amount / MOJO_PER_XSEA}, coefficient: {coefficient}, "
    ...
    async def get_stake_lock_records(
        ...
        if len(stake_records) > 0:
            stake_amount_total = await self.stake_record_store.get_stake_lock_amount_total(end) * MOJO_PER_XSEA
            ...
            for stake in stake_records:
                ...
                amount = 10000 * MOJO_PER_XSEA * (value.stake_amount(stake.amount) / stake_amount_total)
            ...
            for key in stake_rewards_sum:
                stake_rewards[key] = int(stake_rewards_sum[key])
        return stake_rewards
