...
from sea.consensus.pot_iterations import calculate_ip_iters, calculate_sp_iters
...
@streamable
@dataclass(frozen=True)
class BlockRecordOld(Streamable):
    """
    This class is not included or hashed into the blockchain, but it is kept in memory as a more
    efficient way to maintain data about the blockchain. This allows us to validate future blocks,
    difficulty adjustments, etc, without saving the whole header block in memory.
...
def block_record_to_block_record(block_record_bytes: bytes):
    block_record = BlockRecordOld.from_bytes(block_record_bytes)
    return BlockRecord(
        ...
        block_record.challenge_vdf_output,
        block_record.infused_challenge_vdf_output,
        block_record.reward_infusion_new_challenge,
        block_record.challenge_block_info_hash,
        block_record.sub_slot_iters,
        block_record.pool_puzzle_hash,
        block_record.farmer_puzzle_hash,
        block_record.community_puzzle_hash,
        ...
        block_record.prev_transaction_block_hash,
        block_record.fees,
        block_record.reward_claims_incorporated,
        block_record.finished_challenge_slot_hashes,
        block_record.finished_infused_challenge_slot_hashes,
        block_record.finished_reward_slot_hashes,
