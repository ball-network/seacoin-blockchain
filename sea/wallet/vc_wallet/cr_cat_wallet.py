...
from sea.util.misc import VersionedBlob
from sea.wallet.cat_wallet.cat_info import CATCoinData, CRCATInfo
from sea.wallet.cat_wallet.cat_utils import CAT_MOD_HASH, CAT_MOD_HASH_HASH, construct_cat_puzzle
...
from sea.wallet.util.tx_config import TXConfig
...
from sea.wallet.vc_wallet.cr_cat_drivers import (
    CRCAT,
    CRCATMetadata,
    CRCATVersion,
    ProofsChecker,
...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSSeaConnection, coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def _generate_unsigned_spendbundle(
            ...
            if first:
                ...
                if need_sea_transaction:
                    if fee > regular_sea_to_claim:
                        ...
                        sea_tx, xsea_announcement = await self.create_tandem_xsea_tx(
                            fee,
                            uint64(regular_sea_to_claim),
                            tx_config,
                        )
                        assert xsea_announcement is not None
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
                            coin_announcements_to_assert={xsea_announcement.name()},
                            conditions=extra_conditions,
                        )
                else:
                    innersol = self.standard_wallet.make_solution(
        ...
        return (
            SpendBundle(
                [
                    ...
                    *(
                        (
                            spend
                            for bundle in [sea_tx.spend_bundle]
            ...
            [*vc_txs, *([sea_tx] if sea_tx is not None else [])],
        )
    ...
    async def claim_pending_approval_balance(
        ...
        # Select the relevant XSEA coins
        if fee > 0:
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(sea_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
        ...
        if fee > 0:
            ...
            if sea_tx.spend_bundle is None:
                raise RuntimeError("Did not get spendbundle for fee transaction")  # pragma: no cover
            claim_bundle = SpendBundle.aggregate([claim_bundle, sea_tx.spend_bundle])
        else:
            sea_tx = None
        ...
        other_txs: List[TransactionRecord] = [
            ...
            *((dataclasses.replace(sea_tx, spend_bundle=None),) if sea_tx is not None else []),
        ]
