...
from sea.consensus.default_constants import DEFAULT_CONSTANTS
...
from sea.types.coin_spend import compute_additions_with_cost
from sea.types.condition_opcodes import ConditionOpcode
...
from sea.util.errors import Err, ValidationError
...
from sea.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, LegacyCATInfo
from sea.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
    SpendableCAT,
...
from sea.wallet.cat_wallet.lineage_store import CATLineageStore
from sea.wallet.coin_selection import select_coins
from sea.wallet.conditions import Condition, ConditionValidTimes, UnknownCondition, parse_timelock_info
...
from sea.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
from sea.wallet.util.curry_and_treehash import calculate_hash_of_quoted_mod_hash, curry_and_treehash
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            sea_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        sea_tx = dataclasses.replace(sea_tx, spend_bundle=spend_bundle, name=spend_bundle.name())
        await self.standard_wallet.push_transaction(sea_tx)
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSSeaConnection, parent_coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def create_tandem_xsea_tx(
        self,
        fee: uint64,
        ...
        if fee > amount_to_claim:
            sea_coins = await self.standard_wallet.select_coins(
                fee,
                tx_config.coin_selection_config,
            )
            origin_id = list(sea_coins)[0].name()
            [sea_tx] = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
            ...
            origin_id = list(sea_coins)[0].name()
            selected_amount = sum([c.amount for c in sea_coins])
            [sea_tx] = await self.standard_wallet.generate_signed_transaction(
            ...
            message = None
            for spend in sea_tx.spend_bundle.coin_spends:
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some XSEA as part of this
        regular_sea_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_sea_to_claim = payment_amount

        need_sea_transaction = (fee > 0 or regular_sea_to_claim > 0) and (fee - regular_sea_to_claim != 0)
        ...
        for coin in cat_coins:
            ...
            if first:
                ...
                if need_sea_transaction:
                    if fee > regular_sea_to_claim:
                        sea_tx, _ = await self.create_tandem_xsea_tx(
                            fee,
                            uint64(regular_sea_to_claim),
                            tx_config,
        ...
        sea_spend_bundle = SpendBundle([], G2Element())
        if sea_tx is not None and sea_tx.spend_bundle is not None:
            sea_spend_bundle = sea_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if sea_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=sea_tx.confirmed_at_height,
                    created_at_time=sea_tx.created_at_time,
                    to_puzzle_hash=sea_tx.to_puzzle_hash,
                    amount=sea_tx.amount,
                    fee_amount=sea_tx.fee_amount,
                    confirmed=sea_tx.confirmed,
                    sent=sea_tx.sent,
                    spend_bundle=None,
                    additions=sea_tx.additions,
                    removals=sea_tx.removals,
                    wallet_id=sea_tx.wallet_id,
                    sent_to=sea_tx.sent_to,
                    trade_id=sea_tx.trade_id,
                    type=sea_tx.type,
                    name=sea_tx.name,
                    memos=[],
                    valid_times=parse_timelock_info(extra_conditions),
