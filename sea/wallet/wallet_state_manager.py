...
from sea.consensus.block_rewards import (
    calculate_base_farmer_reward,
    calculate_pool_reward,
    calculate_stake_farm_reward,
)
from sea.consensus.coinbase import (
    farmer_parent_id,
    pool_parent_id,
...
from sea.data_layer.dl_wallet_store import DataLayerStore
from sea.pools.pool_puzzles import (
...
from sea.types.announcement import Announcement
...
from sea.types.coin_spend import CoinSpend, compute_additions
from sea.types.mempool_inclusion_status import MempoolInclusionStatus
from sea.types.spend_bundle import SpendBundle
from sea.types.stake_value import STAKE_FARM_MIN
from sea.util.bech32m import encode_puzzle_hash
from sea.util.db_synchronous import db_synchronous_on
...
from sea.util.lru_cache import LRUCache
from sea.util.misc import UInt32Range, UInt64Range, VersionedBlob
from sea.util.path import path_from_root
from sea.util.streamable import Streamable
from sea.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from sea.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, CRCATInfo
from sea.wallet.cat_wallet.cat_utils import CAT_MOD, CAT_MOD_HASH, construct_cat_puzzle, match_cat_puzzle
from sea.wallet.cat_wallet.cat_wallet import CATWallet
from sea.wallet.cat_wallet.dao_cat_wallet import DAOCATWallet
from sea.wallet.conditions import Condition, ConditionValidTimes, parse_timelock_info
from sea.wallet.dao_wallet.dao_utils import (
...
from sea.wallet.dao_wallet.dao_wallet import DAOWallet
from sea.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
from sea.wallet.derivation_record import DerivationRecord
from sea.wallet.derive_keys import (
    _derive_path,
...
from sea.wallet.did_wallet.did_info import DIDCoinData
from sea.wallet.did_wallet.did_wallet import DIDWallet
from sea.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, match_did_puzzle
from sea.wallet.key_val_store import KeyValStore
from sea.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
from sea.wallet.nft_wallet.nft_wallet import NFTWallet
from sea.wallet.nft_wallet.uncurry_nft import NFTCoinData, UncurriedNFT
from sea.wallet.notification_manager import NotificationManager
from sea.wallet.outer_puzzles import AssetType
from sea.wallet.payment import Payment
from sea.wallet.puzzle_drivers import PuzzleInfo
from sea.wallet.puzzles.clawback.drivers import generate_clawback_spend_bundle, match_clawback_puzzle
from sea.wallet.puzzles.clawback.metadata import ClawbackMetadata, ClawbackVersion
from sea.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_hash_for_synthetic_public_key,
)
from sea.wallet.puzzles.stake.drivers import match_stake_puzzle, generate_stake_spend_bundle
from sea.wallet.puzzles.stake.metadata import StakeMetadata, StakeVersion
from sea.wallet.sign_coin_spends import sign_coin_spends
from sea.wallet.singleton import create_singleton_puzzle, get_inner_puzzle_from_singleton, get_singleton_id_from_puzzle
from sea.wallet.trade_manager import TradeManager
from sea.wallet.trading.trade_status import TradeStatus
from sea.wallet.transaction_record import TransactionRecord, TransactionRecordOld
from sea.wallet.uncurried_puzzle import uncurry_puzzle
from sea.wallet.util.address_type import AddressType
...
from sea.wallet.util.puzzle_decorator import PuzzleDecoratorManager
from sea.wallet.util.query_filter import HashFilter
from sea.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from sea.wallet.util.tx_config import TXConfig, TXConfigLoader
from sea.wallet.util.wallet_sync_utils import (
    PeerRequestException,
    fetch_coin_spend_for_coin_state,
    last_change_height_cs,
)
from sea.wallet.util.wallet_types import CoinType, WalletIdentifier, WalletType
from sea.wallet.vc_wallet.cr_cat_drivers import CRCAT, ProofsChecker, construct_pending_approval_state
from sea.wallet.vc_wallet.cr_cat_wallet import CRCATWallet
from sea.wallet.vc_wallet.vc_drivers import VerifiedCredential
from sea.wallet.vc_wallet.vc_store import VCStore
from sea.wallet.vc_wallet.vc_wallet import VCWallet
from sea.wallet.wallet import Wallet
from sea.wallet.wallet_blockchain import WalletBlockchain
from sea.wallet.wallet_coin_record import MetadataTypes, WalletCoinRecord
from sea.wallet.wallet_coin_store import WalletCoinStore
from sea.wallet.wallet_info import WalletInfo
from sea.wallet.wallet_interested_store import WalletInterestedStore
from sea.wallet.wallet_nft_store import WalletNftStore
from sea.wallet.wallet_pool_store import WalletPoolStore
from sea.wallet.wallet_protocol import WalletProtocol
from sea.wallet.wallet_puzzle_store import WalletPuzzleStore
from sea.wallet.wallet_retry_store import WalletRetryStore
from sea.wallet.wallet_transaction_store import WalletTransactionStore
from sea.wallet.wallet_user_store import WalletUserStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSSeaConnection, coin_state: CoinState, fork_height: Optional[uint32]
        ...
        if coin_state.created_height is not None and (
            ...
            or self.is_stake_lock_reward(uint32(coin_state.created_height), coin_state.coin)
        ):
            return None, None
    ...
    async def spend_clawback_coins(
        ...
        for coin, metadata in clawback_coins.items():
            try:
                ...
                # Remove the clawback hint since it is unnecessary for the XSEA coin
        ...
        if fee > 0:
            sea_tx = await self.main_wallet.create_tandem_xsea_tx(
                fee, tx_config, Announcement(coin_spends[0].coin.name(), message)
            )
            assert sea_tx.spend_bundle is not None
            spend_bundle = SpendBundle.aggregate([spend_bundle, sea_tx.spend_bundle])
        assert derivation_record is not None
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        xsea_spam_amount = self.config.get("xsea_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if xsea_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(xsea_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < xsea_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSSeaConnection) -> Optional[bytes32]:
        # Get minter DID
                ...
                async with self.db_wrapper.writer():
                    ...
                    # if the coin has been spent
                        ...
                        if record is None:
                            ...
                            elif self.is_stake_lock_reward(created_height, coin_state.coin):
                                stake_lock_reward = True
                                tx_type = TransactionType.STAKE_LOCK_REWARD.value
                            else:
                            ...
                            record = WalletCoinRecord(
                                ...
                                True,
                                farmer_reward or stake_farm_reward or stake_lock_reward or pool_reward,
                                wallet_identifier.type,
                                wallet_identifier.id,
                            )
    ...
    async def coin_added(
        ...
        elif self.is_stake_lock_reward(height, coin):
            tx_type = TransactionType.STAKE_LOCK_REWARD
        else:
        ...
        coinbase = tx_type in {
            ...
            TransactionType.STAKE_LOCK_REWARD,
        }
    ...
    def is_stake_lock_reward(self, created_height: uint32, coin: Coin) -> bool:
        for i in range(0, 30):
            try_height = created_height - i
            if try_height < 0:
                break
            calculated = stake_lock_reward_parent_id(uint32(try_height), self.constants.GENESIS_CHALLENGE)
            if calculated == coin.parent_coin_info:
                return True
        return False
    ...
    async def handle_stake(
        ...
        if is_recipient is not None:
            ...
            tx_record = TransactionRecord(
                ...
                trade_id=None,
                type=uint32(
                    TransactionType.INCOMING_STAKE_FARM_RECEIVE if metadata.is_stake_farm else
                    TransactionType.INCOMING_STAKE_LOCK_RECEIVE
                ),
                # Use coin ID as the TX ID to mapping with the coin table
                name=coin_record.coin.name(),
    ...
    async def auto_withdraw_stake_coins(self) -> None:
        ...
        stake_lock_coins: Dict[Coin, StakeMetadata] = {}
            ...
            try:
                ...
                if current_timestamp - coin_timestamp >= metadata.time_lock:
                    if metadata.is_stake_farm:
                        ...
                        stake_lock_coins[coin.coin] = metadata
                        if len(stake_lock_coins) >= batch_size:
                            await self.spend_stake_coins(stake_lock_coins, False, tx_fee, tx_config)
                            stake_lock_coins = {}
            except Exception as e:
                self.log.error(f"Failed to withdraw stake coin {coin.coin.name().hex()}: %s", e)
        if len(stake_farm_coins) > 0:
            await self.spend_stake_coins(stake_farm_coins, True, tx_fee, tx_config)
        if len(stake_lock_coins) > 0:
            await self.spend_stake_coins(stake_lock_coins, False, tx_fee, tx_config)
    ...
    async def spend_stake_coins(
        ...
        spend_bundle: SpendBundle = await self.sign_transaction(coin_spends)
        if fee > 0:
            xsea_tx = await self.main_wallet.create_tandem_xsea_tx(
                fee, tx_config, Announcement(coin_spends[0].coin.name(), message)
            )
            assert xsea_tx.spend_bundle is not None
            spend_bundle = SpendBundle.aggregate([spend_bundle, xsea_tx.spend_bundle])
        assert derivation_record is not None
        tx_record = TransactionRecord(
            confirmed_at_height=uint32(0),
            created_at_time=now,
            ...
            trade_id=None,
            type=uint32(
                TransactionType.STAKE_FARM_WITHDRAW if is_stake_farm else TransactionType.STAKE_LOCK_WITHDRAW
            ),
            name=spend_bundle.name(),
            memos=list(compute_memos(spend_bundle).items()),
            valid_times=ConditionValidTimes(),
