...
from sea.wallet.util.wallet_sync_utils import fetch_coin_spend
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSSeaConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSSeaConnection) -> None:
    ...
    async def mint_from_did(
        ...
        if xsea_coins is None:
            xsea_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        ...
        xsea_spend = await self.standard_wallet.sign_transaction(xsea_spends)
