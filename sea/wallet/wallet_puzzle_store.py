...
class WalletPuzzleStore:
    ...
    async def get_first_puzzle_hashes(self, page: int, size: int) -> Set[bytes32]:
        """
        Return a set containing all puzzle_hashes we generated.
        """
        async with self.db_wrapper.reader_no_transaction() as conn:
            rows = await conn.execute_fetchall(
                f"SELECT puzzle_hash FROM derivation_paths ORDER BY derivation_index ASC LIMIT {page*size},{size}"
            )
            return set(bytes32.fromhex(row[0]) for row in rows)
