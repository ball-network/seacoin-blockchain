...
from sea.cmds.farm import farm_cmd
from sea.cmds.init import init_cmd
from sea.cmds.keys import keys_cmd
from sea.cmds.netspace import netspace_cmd
from sea.cmds.passphrase import passphrase_cmd
from sea.cmds.peer import peer_cmd
from sea.cmds.plotnft import plotnft_cmd
from sea.cmds.plots import plots_cmd
...
from sea.util.ssl_check import check_ssl
...
@cli.command("version", help="Show sea version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", help="Runs sea daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by sea start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio

    from sea.daemon.server import async_run_daemon
