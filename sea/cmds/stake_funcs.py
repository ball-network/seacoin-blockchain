...
from sea.types.stake_value import STAKE_FARM_LIST, STAKE_LOCK_LIST
from sea.util.bech32m import encode_puzzle_hash
from sea.util.config import selected_network_address_prefix
from sea.util.errors import CliRpcConnectionError
...
from sea.wallet.util.query_filter import TransactionTypeFilter, HashFilter
...
def print_transaction(
    ...
    if verbose:
        ...
        sea_amount = Decimal(int(tx.amount)) / units["sea"]
        ...
        print(f"Amount {description}: {sea_amount} {address_prefix.upper()}")
...
async def stake_info(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        ...
        if is_stake_farm:
            print(f"Stake Balance: {response['balance'] / units['sea']}")
            print(f"Stake Balance Other: {response['balance_other'] / units['sea']}")
            print(f"Stake Balance Income: {response['balance_income'] / units['sea']}")
            print(f"Stake Balance 24H Exp: {response['balance_exp'] / units['sea']}")
            print(f"Stake Reward: {response['stake_reward'] / units['sea']}")
...
async def stake_send(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        ...
        final_amount = int(amount * units["sea"])
        if not (final_amount / units["sea"]).is_integer():
            print(
...
#             return
#         response = await wallet_client.spend_stake_coins(tx_ids, int(fee * units["sea"]))
#         print(str(response))
...
async def find_pool_nft(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        ...
        print(f"Total Amount: {total_amount} XSEA")
        print(f"Balance Amount: {balance_amount} XSEA")
        print(f"Record Amount: {record_amount} XSEA")
