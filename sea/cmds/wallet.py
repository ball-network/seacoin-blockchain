...
from sea.cmds.check_wallet_db import help_text as check_help_text
from sea.cmds.cmds_util import execute_with_wallet
from sea.cmds.coins import coins_cmd
from sea.cmds.plotnft import validate_fee
...
@wallet_cmd.command("send", help="Send sea to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the transaction, in XSEA",
    type=str,
    default="0",
    show_default=True,
    required=True,
)
@click.option("-t", "--address", help="Address to send the XSEA", type=str, required=True)
@click.option(
...
@click.option(
    "-ma",
    "--min-coin-amount",
    help="Ignore coins worth less then this much XSEA or CAT units",
    type=str,
    required=False,
    default="0",
)
@click.option(
    "-l",
    "--max-coin-amount",
    help="Ignore coins worth more then this much XSEA or CAT units",
    type=str,
    required=False,
    default="0",
)
...
@click.option(
    "-m", "--fee", help="A fee to add to the offer when it gets taken, in XSEA", default="0", show_default=True
)
...
def address_sign_message(wallet_rpc_port: Optional[int], fingerprint: int, address: str, hex_message: str) -> None:
    extra_params: Dict[str, Any] = {"address": address, "message": hex_message, "type": AddressType.XSEA}
    import asyncio
...
@wallet_cmd.command("make_offer", help="Create an offer of XSEA/CATs/NFTs for XSEA/CATs/NFTs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m", "--fee", help="The fee to use when pushing the completed offer, in XSEA", default="0", show_default=True
)
@click.option(
    "--reuse",
    help="Reuse existing address for the offer.",
...
@click.option(
    "-m", "--fee", help="The fee to use when cancelling the offer securely, in XSEA", default="0", show_default=True
)
...
def check_wallet_cmd(ctx: click.Context, db_path: str, verbose: bool) -> None:
    """check, scan, diagnose, fsck Sea Wallet DBs"""
    import asyncio

    from sea.cmds.check_wallet_db import scan
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in XSEA.",
    type=str,
    default="0",
    show_default=True,
...
@click.option("-m", "--fee", help="The fee for the transaction, in XSEA", type=str)
...
@click.option("-m", "--fee", help="Blockchain fee for mint transaction, in XSEA", type=str, required=False)
def _mint_vc(
    wallet_rpc_port: Optional[int],
    fingerprint: int,
    did: str,
...
@click.option("-m", "--fee", help="Blockchain fee for update transaction, in XSEA", type=str, required=False)
@click.option(
...
@click.option("-m", "--fee", help="Blockchain fee for revocation transaction, in XSEA", type=str, required=False)
@click.option(
    "--reuse-puzhash/--generate-new-puzhash",
    help="Send the VC back to the same puzzle hash it came from (ignored if --new-puzhash is specified)",
    default=False,
...
@wallet_cmd.command("find_pool_nft", short_help="fnd nft sea")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-l",
    "--launcher-id",
    help="Set the launcher id, you should get it from sea wallet",
    type=str,
    required=True
)
@click.option(
