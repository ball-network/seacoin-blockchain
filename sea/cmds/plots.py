...
from sea.plotting.util import add_plot_directory, validate_plot_size

log = logging.getLogger(__name__)


def show_plots(root_path: Path):
    from sea.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'sea plots add -d [dir]' and remove with"
        + " 'sea plots remove -d [dir]'"
        + " Scan and check plots with 'sea plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    from sea.util.sea_logging import initialize_logging

    root_path: Path = ctx.obj["root_path"]
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'sea init'")
...
def create_cmd(
    ...
    from sea.plotting.create_plots import create_plots, resolve_plot_keys
    from sea.plotting.util import Params
...
def check_cmd(
    ...
    from sea.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from sea.plotting.util import remove_plot_directory
