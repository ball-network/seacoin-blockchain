...
def create_sea_directory(
    ...
    """
    This function creates a new sea directory and returns a heavily modified config,
    suitable for use in the simulator.
    """
    from sea.cmds.init_funcs import sea_init

    if not sea_root.is_dir() or not Path(sea_root / "config" / "config.yaml").exists():
        # create sea directories & load config
        sea_init(sea_root, testnet=True, fix_ssl_permissions=True)
        config: Dict[str, Any] = load_config(sea_root, "config.yaml")
        ...
        config = load_config(sea_root, "config.yaml")
    # simulator overrides
    ...
    save_config(sea_root, "config.yaml", config)
    return config


def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `sea keys show`.
    """
...
def generate_and_return_fingerprint(mnemonic: Optional[str] = None) -> int:
    ...
    """
    from sea.util.keychain import generate_mnemonic
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    from sea.simulator.start_simulator import PLOT_SIZE, PLOTS

    farming_puzzle_hash = decode_puzzle_hash(config["simulator"]["farming_address"])
    os.environ["SEA_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    if fingerprint is None:
        ...
        return
    # create sea directory & get config.
    print("Creating sea directory & config...")
    config = create_sea_directory(root_path, fingerprint, farming_address, plot_directory, auto_farm, docker_mode)
    ...
    sys.argv[0] = str(Path(sys.executable).parent / "sea")  # fix path for tests
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your SEA_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    """
    from sea.cmds.show_funcs import print_blockchain_state
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        if node_client is not None:
            ...
            # chain status ( basically sea show -s)
            ...
            print(
                ...
                f"with a balance of: "
                f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['sea']} TXSEA."
            )
            if show_addresses:
                print("All Addresses: ")
