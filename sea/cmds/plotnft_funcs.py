...
from sea.cmds.cmds_util import get_any_service_client, transaction_status_msg, transaction_submitted_msg
from sea.cmds.units import units
from sea.cmds.wallet_funcs import print_balance, wallet_coin_unit
from sea.pools.pool_config import PoolWalletConfig, load_pool_config, update_pool_config
from sea.pools.pool_wallet_info import PoolSingletonState, PoolWalletInfo
...
async def create(args: Dict[str, Any], wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojos = uint64(int(fee * units["sea"]))
    ...
    if user_input.lower() == "y" or user_input.lower() == "yes":
        try:
            ...
            print(f"Error creating plot NFT: {e}\n    Please start both farmer and wallet with:  sea start -r farmer")
        return
    print("Aborting.")
