...
from sea.cmds.init_funcs import check_keys, sea_full_version_str, sea_init
from sea.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from sea.daemon.keychain_server import KeychainServer, keychain_commands
from sea.daemon.windows_signal import kill
from sea.plotters.plotters import get_available_plotters
from sea.plotting.util import add_plot_directory
from sea.server.server import ssl_context_for_server
from sea.util.beta_metrics import BetaMetricsLogger
from sea.util.sea_logging import initialize_service_logging
from sea.util.config import load_config
from sea.util.errors import KeychainCurrentPassphraseIsInvalid
from sea.util.json_util import dict_to_json_str
from sea.util.keychain import Keychain, passphrase_requirements, supports_os_passphrase_storage
from sea.util.lock import Lockfile, LockfileError
from sea.util.network import WebServer
from sea.util.service_groups import validate_service
from sea.util.setproctitle import setproctitle
from sea.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Sea.")
    quit()
...
if getattr(sys, "frozen", False):
    name_map = {
        "sea": "sea",
        "sea_data_layer": "start_data_layer",
        "sea_data_layer_http": "start_data_layer_http",
        "sea_wallet": "start_wallet",
        "sea_full_node": "start_full_node",
        "sea_harvester": "start_harvester",
        "sea_farmer": "start_farmer",
        "sea_introducer": "start_introducer",
        "sea_timelord": "start_timelord",
        "sea_timelord_launcher": "timelord_launcher",
        "sea_full_node_simulator": "start_simulator",
        "sea_seeder": "start_seeder",
        "sea_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Sea will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["sea", "plotters", plotter]

        if plotter == "bladebit":
            # plotter command must be either
            # 'sea plotters bladebit ramplot' or 'sea plotters bladebit diskplot'
            plot_type = request["plot_type"]
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered SEA_ROOT
    os.environ["SEA_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with SEA_ROOT: {os.environ['SEA_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in sea_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    sea_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("sea_daemon")
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"seacoin-blockchain version: {sea_full_version_str()}")
