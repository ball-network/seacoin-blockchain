...
from sea.daemon.server import service_launch_lock_path
from sea.rpc.rpc_server import RpcApiProtocol, RpcServer, RpcServiceProtocol, start_rpc_server
from sea.server.api_protocol import ApiProtocol
from sea.server.sea_policy import set_sea_policy
from sea.server.outbound_message import NodeType
from sea.server.server import SeaServer
from sea.server.ssl_context import sea_ssl_ca_paths, private_ssl_ca_paths
from sea.server.upnp import UPnP
from sea.server.ws_connection import WSSeaConnection
from sea.types.peer_info import PeerInfo, UnresolvedPeerInfo
...
from sea.util.network import resolve
...
class Service(Generic[_T_RpcServiceProtocol, _T_ApiProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSSeaConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"seacoin-blockchain version: {sea_full_version_str()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        sea_ca_crt, sea_ca_key = sea_ssl_ca_paths(root_path, self.config)
    ...
    async def manage(self, *, start: bool = True) -> AsyncIterator[None]:
        ...
        async with contextlib.AsyncExitStack() as async_exit_stack:
            try:
                ...
                self._log.info("Waiting for SeaServer to be closed")
    ...
    async def setup_process_global_state(self, signal_handlers: SignalHandlers) -> None:
        ...
        proctitle_name = f"sea_{self._service_name}"
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    with log_exceptions(log=log, message="fatal uncaught exception"):
        if connection_limit is not None:
            set_sea_policy(connection_limit)
