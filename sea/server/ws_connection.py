...
from sea.protocols.protocol_state_machine import message_response_ok
from sea.protocols.protocol_timing import (
    API_EXCEPTION_BAN_SECONDS,
    CONSENSUS_ERROR_BAN_SECONDS,
    INTERNAL_PROTOCOL_ERROR_BAN_SECONDS,
)
from sea.protocols.shared_protocol import Capability, Error, Handshake
from sea.server.api_protocol import ApiProtocol
from sea.server.capabilities import known_active_capabilities
from sea.server.outbound_message import Message, NodeType, make_msg
from sea.server.rate_limits import RateLimiter
from sea.types.blockchain_format.sized_bytes import bytes32
from sea.types.peer_info import PeerInfo
from sea.util.api_decorators import get_metadata
from sea.util.errors import ApiError, ConsensusError, Err, ProtocolError, TimestampError
from sea.util.ints import int16, uint8, uint16
from sea.util.log_exceptions import log_exceptions

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from sea.util.network import class_for_type, is_localhost
...
ConnectionCallback = Callable[["WSSeaConnection"], Awaitable[None]]
...
class WSSeaConnection:
    ...
    sea_full_version: Optional[Version] = None
    ...
    def create(
        ...
        return cls(
            ...
            session=session,
            sea_full_version=Version(sea_full_version_str()),
        )
    ...
    def is_old_version(self) -> bool:
        return Version(self.version) < self.sea_full_version
