...
class SeaSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _sea_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class SeaProactor(IocpProactor):
        ...
        async def _sea_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._sea_accept(listener)
                except WindowsError as exc:  # pylint: disable=E0602
                    if exc.winerror not in (_winapi.ERROR_NETNAME_DELETED, _winapi.ERROR_OPERATION_ABORTED):
                        raise

        def _sea_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            ...
            async def accept_coro(self: SeaProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._sea_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class SeaProactorEventLoop(ProactorEventLoop):
...
class SeaPolicy(asyncio.DefaultEventLoopPolicy):
    def new_event_loop(self) -> asyncio.AbstractEventLoop:
        # overriding https://github.com/python/cpython/blob/v3.11.0/Lib/asyncio/events.py#L689-L695
        if sys.platform == "win32":
            loop_factory = SeaProactorEventLoop
        else:
            loop_factory = SeaSelectorEventLoop

        return loop_factory()


def set_sea_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(SeaPolicy())
