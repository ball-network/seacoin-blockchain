...
from sea.protocols.protocol_state_machine import message_requires_reply
from sea.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS
from sea.protocols.shared_protocol import protocol_version
from sea.server.api_protocol import ApiProtocol
from sea.server.introducer_peers import IntroducerPeers
from sea.server.outbound_message import Message, NodeType
from sea.server.ssl_context import private_ssl_paths, public_ssl_paths
from sea.server.ws_connection import ConnectionCallback, WSSeaConnection
from sea.types.blockchain_format.sized_bytes import bytes32
from sea.types.peer_info import PeerInfo
from sea.util.errors import Err, ProtocolError
from sea.util.ints import uint16
from sea.util.network import WebServer, is_in_network, is_localhost, is_trusted_peer
from sea.util.ssl_check import verify_ssl_certs_and_keys
...
class SeaServer:
    ...
    all_connections: Dict[bytes32, WSSeaConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        sea_ca_crt_key: Tuple[Path, Path],
        name: str = __name__,
    ) -> SeaServer:
        ...
        sea_ca_crt_path, sea_ca_key_path = sea_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                ca_cert=sea_ca_crt_path,
                ca_key=sea_ca_key_path,
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSSeaConnection] = []
    ...
    async def start(
        ...
        if self.webserver is not None:
            raise RuntimeError("SeaServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSSeaConnection] = None
        try:
            connection = WSSeaConnection.create(
    ...
    async def connection_added(
        self, connection: WSSeaConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    def connection_closed(self, connection: WSSeaConnection, ban_time: int, closed_connection: bool = False) -> None:
    ...
    def is_trusted_peer(self, peer: WSSeaConnection, trusted_peers: Dict[str, Any]) -> bool:
