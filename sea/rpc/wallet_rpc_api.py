...
from sea.consensus.block_rewards import calculate_base_farmer_reward, MOJO_PER_XSEA
...
from sea.pools.pool_puzzles import create_p2_singleton_puzzle, POOL_OUTER_MOD_HASH
...
from sea.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
from sea.rpc.util import tx_endpoint
from sea.server.outbound_message import NodeType, make_msg
from sea.server.ws_connection import WSSeaConnection
from sea.simulator.simulator_protocol import FarmNewBlockProtocol
from sea.types.announcement import Announcement
from sea.types.blockchain_format.coin import Coin, coin_as_list
from sea.types.blockchain_format.program import Program
from sea.types.blockchain_format.serialized_program import SerializedProgram
...
from sea.types.spend_bundle import SpendBundle, estimate_fees
from sea.types.stake_value import (
    STAKE_FARM_PREFIX,
    STAKE_FARM_LIST,
    STAKE_FARM_MIN,
    STAKE_FARM_COUNT,
...
from sea.util.byte_types import hexstr_to_bytes
from sea.util.config import load_config, str2bool
from sea.util.errors import KeychainIsLocked
from sea.util.ints import uint16, uint32, uint64, uint128
from sea.util.keychain import bytes_to_mnemonic, generate_mnemonic
from sea.util.misc import UInt32Range
from sea.util.path import path_from_root
from sea.util.streamable import Streamable, streamable
from sea.util.ws_message import WsRpcMessage, create_payload_dict
from sea.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from sea.wallet.cat_wallet.cat_info import CRCATInfo
from sea.wallet.cat_wallet.cat_wallet import CATWallet
from sea.wallet.cat_wallet.dao_cat_info import LockedCoinInfo
from sea.wallet.cat_wallet.dao_cat_wallet import DAOCATWallet
from sea.wallet.conditions import Condition
from sea.wallet.dao_wallet.dao_info import DAORules
...
from sea.wallet.did_wallet.did_info import DIDCoinData, DIDInfo
from sea.wallet.did_wallet.did_wallet import DIDWallet
from sea.wallet.did_wallet.did_wallet_puzzles import (
    DID_INNERPUZ_MOD,
...
from sea.wallet.puzzles.clawback.metadata import AutoClaimSettings, ClawbackMetadata
from sea.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from sea.wallet.puzzles.stake.metadata import AutoWithdrawStakeSettings
from sea.wallet.singleton import (
    SINGLETON_LAUNCHER_PUZZLE_HASH,
    create_singleton_puzzle,
    get_inner_puzzle_from_singleton,
)
from sea.wallet.trade_record import TradeRecord
from sea.wallet.trading.offer import Offer
...
from sea.wallet.util.tx_config import DEFAULT_TX_CONFIG, CoinSelectionConfig, CoinSelectionConfigLoader, TXConfig
...
from sea.wallet.vc_wallet.cr_cat_drivers import ProofsChecker
...
class WalletRpcApi:
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: WSSeaConnection, coin_id: bytes32, latest: bool = True
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xsea_target_address")
        pool_target = config["pool"].get("xsea_target_address")
    ...
    async def create_new_wallet(
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                from sea.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def verify_signature(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        # signatures made by `sea keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def cancel_offers(
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "xsea")
        ...
        key = None
        if asset_id is not None and asset_id != "xsea":
    ...
    async def did_get_info(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.XSEA.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(
        ...
        xsea_coin_list = request.get("xsea_coins", None)
        xsea_coins = None
        if xsea_coin_list:
            xsea_coins = {Coin.from_json_dict(xsea_coin) for xsea_coin in xsea_coin_list}
        xsea_change_target = request.get("xsea_change_target", None)
        if xsea_change_target is not None:
            if xsea_change_target[:2] == "xsea":
                xsea_change_ph = decode_puzzle_hash(xsea_change_target)
            else:
                xsea_change_ph = bytes32(hexstr_to_bytes(xsea_change_target))
        else:
            xsea_change_ph = None
        ...
        if mint_from_did:
            txs = await nft_wallet.mint_from_did(
                ...
                xsea_coins=xsea_coins,
                xsea_change_ph=xsea_change_ph,
            ...
            txs = await nft_wallet.mint_from_xsea(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
    ...
    async def get_farmed_amount(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        for record in tx_records:
            ...
            elif record.type == TransactionType.STAKE_LOCK_REWARD.value:
                stake_lock_reward_amount += record.amount
    ...
    async def stake_info(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        coin_records = await self.service.wallet_state_manager.get_all_transactions_by_confirmed(
            wallet_id,
            TransactionType.INCOMING_STAKE_FARM_RECEIVE.value if is_stake_farm
            else TransactionType.INCOMING_STAKE_LOCK_RECEIVE.value,
            True,
        )
        current_timestamp = self.service.wallet_state_manager.blockchain.get_latest_timestamp()
        stake_reward = await self.service.wallet_state_manager.tx_store.get_stake_rewards(is_stake_farm)
        balance = 0
        balance_exp = 0
        if is_stake_farm:
            balance_other = 0
    ...
    async def stake_send(
        ...
        if is_stake_farm:
            if amount < STAKE_FARM_MIN:
                return {"success": False, "error": f"stake farm must more then {int(STAKE_FARM_MIN/MOJO_PER_XSEA)} XSEA"}
            if address != "":
            ...
            if amount < STAKE_LOCK_MIN:
                return {"success": False, "error": f"stake lock must more then {int(STAKE_LOCK_MIN/MOJO_PER_XSEA)} XSEA"}
            if address != "":
                selected_network = self.service.config["selected_network"]
                expected_prefix = self.service.config["network_overrides"]["config"][selected_network]["address_prefix"]
                if address[0: len(expected_prefix)] != expected_prefix:
                    return {"success": False, "error": f"Unexpected stake lock address Prefix: {expected_prefix}"}
                puzzle_hash: bytes32 = decode_puzzle_hash(address)
            else:
