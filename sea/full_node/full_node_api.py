...
from sea.consensus.block_creation import create_unfinished_block
from sea.consensus.block_record import BlockRecord
from sea.consensus.pot_iterations import calculate_ip_iters, calculate_iterations_quality, calculate_sp_iters
from sea.full_node.bundle_tools import best_solution_generator_from_template, simple_solution_generator
from sea.full_node.fee_estimate import FeeEstimate, FeeEstimateGroup, fee_rate_v2_to_v1
from sea.full_node.fee_estimator_interface import FeeEstimatorInterface
from sea.full_node.mempool_check_conditions import get_name_puzzle_conditions, get_puzzle_and_solution_for_coin
from sea.full_node.signage_point import SignagePoint
from sea.full_node.tx_processing_queue import TransactionQueueFull
from sea.protocols import farmer_protocol, full_node_protocol, introducer_protocol, timelord_protocol, wallet_protocol
from sea.protocols.full_node_protocol import RejectBlock, RejectBlocks
...
from sea.types.block_protocol import BlockInfo
from sea.types.blockchain_format.coin import Coin, hash_coin_ids
from sea.types.blockchain_format.pool_target import PoolTarget
from sea.types.blockchain_format.proof_of_space import verify_and_get_quality_string
...
from sea.util.full_block_utils import header_block_from_block
...
from sea.util.limited_semaphore import LimitedSemaphoreFullError
from sea.util.merkle_set import MerkleSet
...
class FullNodeAPI:
    ...
    async def request_peers(
        self, _request: full_node_protocol.RequestPeers, peer: WSSeaConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: WSSeaConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: WSSeaConnection
    ) -> Optional[Message]:
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: WSSeaConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: WSSeaConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: WSSeaConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: WSSeaConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: WSSeaConnection
    ) -> Optional[Message]:
        """
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: WSSeaConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: WSSeaConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: WSSeaConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: WSSeaConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
    ...
    async def new_compact_vdf(
        self, request: full_node_protocol.NewCompactVDF, peer: WSSeaConnection, request_bytes: bytes = b""
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @api_request(peer_required=True)
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSSeaConnection) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def register_interest_in_puzzle_hash(
        self, request: wallet_protocol.RegisterForPhUpdates, peer: WSSeaConnection
    ) -> Message:
    ...
    async def register_interest_in_coin(
        self, request: wallet_protocol.RegisterForCoinUpdates, peer: WSSeaConnection
    ) -> Message:
    ...
    async def request_staking_coefficients(
        ...
        staking_height = self.full_node.blockchain.get_peak_height()
        if staking_height is None:
            staking_height = uint32(0)
        for farmer_public_key in request.farmer_public_keys:
            coefficient = await self.full_node.blockchain.get_staking_coefficient(staking_height, farmer_public_key)
            staking_coefficients.append((farmer_public_key, coefficient))
        return make_msg(
            ProtocolMessageTypes.respond_staking_coefficients,
            farmer_protocol.FarmerStakingCoefficients(staking_height, staking_coefficients),
        )
