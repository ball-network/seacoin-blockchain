...
from sea.util.full_block_utils import GeneratorBlockInfo, block_info_from_block, generator_from_block
...
class BlockStore:
    ...
    async def create(cls, db_wrapper: DBWrapper2, *, use_cache: bool = True) -> BlockStore:
        ...
        async with self.db_wrapper.writer_maybe_transaction() as conn:
            ...
            log.info("DB: Creating index farm_puzzle_hash")
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS farm_puzzle_hash on full_blocks(farm_puzzle_hash)"
            )
        ...
        async with self.db_wrapper.writer_maybe_transaction() as conn:
            await conn.execute(
                ...
                (
                    ...
                    compress(block),
                    bytes(block_record),
                    bytes(block_record.farm_puzzle_hash),
                ),
            )
