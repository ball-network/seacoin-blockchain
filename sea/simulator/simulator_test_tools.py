...
from sea.simulator.start_simulator import async_main as start_simulator_main
...
from sea.util.config import create_default_sea_config, load_config, save_config
...
def create_config(
    ...
    create_default_sea_config(sea_root)
    create_all_ssl(
        sea_root,
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "txsea")
...
async def get_full_sea_simulator(
    ...
    """
    A sea root Path is required.
    The sea root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    This test is fully interdependent and can be used without the rest of the sea test suite.
    ...
    with Lockfile.create(daemon_launch_lock_path(sea_root)):
        ...
        crt_path = sea_root / config["daemon_ssl"]["private_crt"]
        key_path = sea_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = sea_root / config["private_ssl_ca"]["crt"]
