...
# Gets the version from the result of "sea version"
...
def get_sea_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["sea", "version"], capture_output=True)
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/seacoin-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/seacoin-blockchain-gui/packages/gui/package.json")
