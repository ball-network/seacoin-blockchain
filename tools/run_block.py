...
"""
run_block: Convert an encoded FullBlock from the SeaCoin blockchain into a list of transactions

As input, takes a file containing a [FullBlock](../sea/types/full_block.py) in json format
...
$ca_root is the directory containing your current Sea config files
$hash is the header_hash of the [BlockRecord](../sea/consensus/block_record.py)
...
The information for each spend is an "NPC" (Name, Puzzle, Condition):
        ...
        "conditions": a list of condition expressions, as in [condition_opcodes.py](../sea/types/condition_opcodes.py)
...
from sea.types.condition_with_args import ConditionWithArgs
...
from sea.wallet.cat_wallet.cat_utils import match_cat_puzzle
