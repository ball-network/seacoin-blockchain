...
git submodule update --init seacoin-blockchain-gui

Set-Location -Path ".\seacoin-blockchain-gui" -PassThru
...
# Other than `seacoin-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @sea-network packages to make cache slim"
...
# Remove `packages/gui/node_modules/@sea-network` because it causes an error on later `electron-packager` command
#Remove-Item "@sea-network" -Recurse -Force
